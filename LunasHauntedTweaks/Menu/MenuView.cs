...
namespace HMMLunasTweaks.Menu
{
    class MenuView : MonoBehaviour
    {
        ...
        private void LoadMenu()
        {
            ...
            string[,] textInfo = new string[,] { { "MenuTitle", $"Luna's HMM Tweaks v{PluginInfo.Version}" }, { "PageText", "Pages" } };
            ...
            for (loopIndex = 0; loopIndex < positions?.Length; loopIndex++)
            {
                ...
                if (go != null)
                {
                    SetLocal(go.transform, positions[loopIndex], new Vector3(0.0036f, 0.0018f, 0f), zeroRotation);
            ...
            Vector3 buttonTextScale = new Vector3(0.004f, 0.016f, 1f);
            ...
            for (loopIndex = 0; loopIndex < pageTriggers?.Length; loopIndex++)
            {
                ...
                SetLocal(go?.transform, buttonPos, buttonScale, zeroRotation);
                SetLocal(textObject?.transform, buttonTextPos, new Vector3(0.03f, 0.03f, 1f), zeroRotation, go?.transform);
        ...
        private GameObject CreateButton(string name)
        {
            ...
            go.name = name ?? "HauntedModMenuButton";
